...
module.exports = {
  ...
  async findUser(req, res) {
    try {
      ...
      res
        .status(401)
        .json({ message: `No se encontró el usuario con id ${id}` });
    }
  },
  ...
  async filter(req, res) {
    ...
    try {
      if (info.categorie && info.departmentID && info.city) {
        pilot = await Pilot.find({
          $and: [
            { "services.name": info.categorie },
        ...
        pilot = await Pilot.find({ "services.name": info.categorie }).populate(
          "media"
        );
        ...
        pilot = await Pilot.find({}).populate("media");
      }
      res.status(200).json(pilot);
    } catch (err) {
      console.log(err);
